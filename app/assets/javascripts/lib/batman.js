(function() {
  ...
  Batman.DelegatingStateMachine = (function(_super) {
    ...
    DelegatingStateMachine.name = 'DelegatingStateMachine';
    ...
    App.run = function() {
      ...
      if (layout) {
        if (typeof layout === 'string') {
          layoutClass = this[helpers.camelize(layout) + 'View'];
        }
      } else {
        if (layout !== null) {
  ...
  Batman.Controller = (function(_super) {
    ...
    Controller.prototype.dispatch = function(action, params) {
      var name, redirectTo, yieldObject, _ref;
      if (params == null) {
        params = {};
      }
      ...
      _ref = Batman.DOM.Yield.yields;
      for (name in _ref) {
        yieldObject = _ref[name];
        if (!this._renderedYields[name]) {
          yieldObject.clear();
        }
      }
    ...
    Controller.prototype.redirect = function(url) {
      ...
      if (frame) {
        if (frame.actionTaken) {
          developer.warn("Warning! Trying to redirect but an action has already be taken during " + (this.get('routingKey')) + "." + (frame.action || this.get('action')) + "}");
        }
    ...
    Controller.prototype.render = function(options) {
      var action, frame, view, _ref, _ref1, _ref2, _ref3,
        _this = this;
      if (options == null) {
        options = {};
      }
      ...
      if (options) {
        options.into || (options.into = 'main');
      }
      if (frame && frame.actionTaken && this._renderedYields[options.into]) {
        developer.warn("Warning! Trying to render but an action has already be taken during " + (this.get('routingKey')) + "." + action + " on yield " + options.into);
      }
      if (frame != null) {
        frame.actionTaken = true;
      }
      ...
      if ((_ref1 = this._renderedYields) != null) {
        _ref1[options.into] = true;
      }
      if (!options.view) {
        options.viewClass || (options.viewClass = ((_ref2 = Batman.currentApp) != null ? _ref2[helpers.camelize("" + (this.get('routingKey')) + "_" + action + "_view")] : void 0) || Batman.View);
        options.context || (options.context = this.get('_renderContext'));
        options.source || (options.source = helpers.underscore(this.get('routingKey') + '/' + action));
      ...
      if (view) {
        ...
        view.on('ready', function() {
          ...
          return (_ref4 = Batman.currentApp) != null ? _ref4.allowAndFire('ready') : void 0;
        });
      }
      return view;
    };
  ...
  Batman.Model = (function(_super) {
    ...
    Model.LifecycleStateMachine = (function(_super1) {
      ...
      LifecycleStateMachine.name = 'LifecycleStateMachine';
    ...
    Model.classAccessor('resourceName', {
      get: function() {
        if (this.resourceName != null) {
          ...
          if (Batman.config.minificationErrors) {
            developer.error("Please define " + ($functionName(this)) + ".resourceName in order for your model to be minification safe.");
          }
    ...
    Model.InstanceLifecycleStateMachine = (function(_super1) {
      ...
      InstanceLifecycleStateMachine.name = 'InstanceLifecycleStateMachine';
    ...
    Association.prototype.getRelatedModel = function() {
      ...
      developer["do"](function() {
        if ((Batman.currentApp != null) && !relatedModel) {
          return developer.warn("Related model " + className + " hasn't loaded yet.");
        }
      });
      return relatedModel;
    };
  ...
  Batman.RestStorage = (function(_super) {
    ...
    RestStorage.ModelMixin = $extend({}, RestStorage.BaseMixin, {
      urlNestsUnder: function() {
        var childSegment, key, keys, parents, _i, _len;
  ...
  Batman.View = (function(_super) {
    ...
    View.YieldStorage = (function(_super1) {
      ...
      YieldStorage.name = 'YieldStorage';
  ...
  Batman.DOM = {
    ...
    Yield: Yield = (function(_super) {
      ...
      function Yield() {
        return Yield.__super__.constructor.apply(this, arguments);
      }
      ...
      Yield.prototype.clear = Yield.queued(function() {
        ...
        _ref = Array.prototype.slice.call(this.containerNode.childNodes);
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          child = _ref[_i];
          _results.push($removeOrDestroyNode(child));
        }
        return _results;
      });

      Yield.prototype.append = Yield.queued(function(node) {
        return $appendChild(this.containerNode, node, true);
      });
